...
matching::matching(Graph &query_graph, Graph &data_graph,
        ...
        : query_(query_graph), data_(data_graph), max_num_results_(max_num_results),
          ...
          num_intermediate_results_after_index_check_(0ul), num_intermediate_results_after_joinability_check_(0ul),
          num_intermediate_results_after_visit_check_(0ul), num_intermediate_results_with_empty_candidate_set_(0ul),
...
void matching::deleteEdge(uint v1, uint v2) {
...
void matching::PrintCounter() {
    ...
    std::cout << num_intermediate_results_after_joinability_check_
              << " intermediate results after joinability check.\n";
    std::cout << num_intermediate_results_after_visit_check_ << " intermediate results after visit check.\n";
    std::cout << num_intermediate_results_with_empty_candidate_set_
              << " intermediate results with empty candidate set.\n";
