...
Graphflow::Graphflow(Graph &query_graph, Graph &data_graph,
        ...
        : matching(query_graph, data_graph, max_num_results,
          ...
          suffixMax(query_graph.NumVertices()+1, 0), isolatedMax(query_graph.NumVertices(), -1),
...
void Graphflow::FindMatches(uint flag, uint order_index, uint depth, std::vector<uint> m, size_t &num_results,
    ...
    //??????????????????????u??????????????????u_other????????ß≥??u_other?u_min
    //???u_min????????????u??u_min????????????????????ß÷?u_min???????????ßµ?????ß“????u???????????????
    //?????????????
...
void Graphflow::updateGlobalEdgeIndex(uint mindex,uint candidate_u,uint candidate_u_pos,uint v1,uint v2,uint candidate_u_neighbor,float weight){
    StarGraph*s=globalStarIndex[mindex][candidate_u_pos];
    std::vector<ForwardNeighbor*>&queryVertex=s->GetqueryVertex();
    bool flag=0;
    if(v1>v2)
        swap(v1,v2);
    ...
    for(auto fneighbor:queryVertex){
        if(fneighbor->GetVetexId()==candidate_u_neighbor){
            ...
            std::tuple<uint,uint,uint>key(v1label,v2label,elabel);
            auto it=data_.globalEdgeIndex.find(key);
            if(it!=data_.globalEdgeIndex.end()){
              if(weight>it->second){
                  //update globalEdgeIndex stargraph
                  it->second=weight;
                  data_.globalEdgeTodataVertex[key]=pair_v;
                  flag=1;
              }
            }
        }
    ...
    if(flag==1){
        ...
        for(auto fneighbor:queryVertex){
            ...
            if(v1label>v2label)
                swap(v1label,v2label);
            std::tuple<uint,uint,uint>key(v1label,v2label,elabel);
            sumweight+=data_.globalEdgeIndex[key];
        }
        s->setStarMaxWeight(sumweight);
    }
}
    ...
    for (auto ma:match) {
        ...
        if (v1label != v2label) {
            ...
            if (this->LabelFilter(v1, u1) && this->LabelFilter(v2, u2)) {
                ...
                SearchMatchesWithGlobalIndexEdge(ma, v1, v2, weight, u1, u2, positive);
            }
        } else {
            for (int i = 0; i < 2; i++) {
                if (this->LabelFilter(v1, u1) && this->LabelFilter(v2, u2)) {
...
void Graphflow::AddEdgeWithEdgeIndex(uint v1, uint v2, uint label, float weight, uint timestamp) {
    total_update_globalIndex_time.StartTimer();
    numAddTopk = 0;
    allMatchFind = 0;
    data_.AddEdge(v1, v2, label, weight, timestamp, 1);
...
void Graphflow::AddEdge(uint v1, uint v2, uint label, float weight, uint timestamp) {
    ...
    for (auto m: match) {
        ...
        if (v1label != v2label) {
            ...
            //todo
            if (this->LabelFilter(v1, u1) && this->LabelFilter(v2, u2)) {
                SearchMatchesWithEdge(m,v1,v2,weight,u1,u2,positive);
            }
        } else {
            for (int i = 0; i < 2; i++) {
                if (this->LabelFilter(v1, u1) && this->LabelFilter(v2, u2)) {
    ...
    /*  start=Get_Time();
      ...
      for (uint i = 0; i < query_.NumEdges(); i++) {
          ...
          if (
              ...
              visited_[v1] = true;
              visited_[v2] = true;
              uint tm = data_.GetEdgeTime(s, t);//??ß≥???????????????????
        ...
        while (lower != neighbors.end()) {
            ...
            for (auto m: match) {
                ...
                if (v1label != v2label) {
                    ...
                    //todo
                    if (this->LabelFilter(v1, u1) && this->LabelFilter(v2, u2)) {
                        SearchMatchesWithEdge(m,v1,v2,weight,u1,u2,negative);
                    }
                } else {
                    for (int i = 0; i < 2; i++) {
                        if (this->LabelFilter(v1, u1) && this->LabelFilter(v2, u2)) {
...
void Graphflow::RemoveEdgeWithGlobal(uint v1, uint v2, uint label) {
    //?????
    total_delete_update_time.StartTimer();
    allMatchFind = 0;
    uint v1label = data_.GetVertexLabel(v1);
    ...
    uint n = query_.NumEdges();
    vector<int> match = EdgeisInMatchOrder(v1, v2, v1label, v2label, label);
    deleteUpdateStarIndexWithGlobal(v1, v2, match);
    total_delete_update_time.StopTimer();
    total_delete_time.StartTimer();
    bool flag = deleteMatchRecordWithEdge(v1, v1label, v2, v2label, label, match);
    if (!flag) {
        ...
        while (lower != neighbors.end()) {
            ...
            for (auto m: match) {
                ...
                if (v1label != v2label) {
                    ...
                    //todo
                    if (this->LabelFilter(v1, u1) && this->LabelFilter(v2, u2)) {
                        SearchMatchesWithGlobalIndexEdge(m,v1,v2,weight,u1,u2,negative);
                    }
                } else {
                    for (int i = 0; i < 2; i++) {
                        if (this->LabelFilter(v1, u1) && this->LabelFilter(v2, u2)) {
...
void Graphflow::RemoveEdgeWithEdgeIndex(uint v1, uint v2, uint label) {
    //?????
    total_delete_update_time.StartTimer();
    allMatchFind = 0;
    uint v1label = data_.GetVertexLabel(v1);
    ...
    uint n = query_.NumEdges();
    vector<int> match = EdgeisInMatchOrder(v1, v2, v1label, v2label, label);
    deleteUpdateStarIndexWithEdgeIndex(v1, v2, match);
    total_delete_update_time.StopTimer();
    total_delete_time.StartTimer();
    bool flag = deleteMatchRecordWithEdge(v1, v1label, v2, v2label, label, match);
    if (!flag) {
...
void Graphflow::SearchMatchesWithGlobalIndexEdge(uint m, uint v1, uint v2, uint weight, uint u1, uint u2, searchType type) {
    if(isInsert)
    {
        IsearchSpace+=2;
    }
    ...
    if(topKSet.size()==k){
        ...
        backWeight+=weight;
        uint n=query_.NumVertices();
        backWeight=backWeight/(sqrt(n)*(n-1));
        if(backWeight<topKSet.back()->getDensity())
        {
            return ;
        }
    }
...
void Graphflow::searchMatchesWithGLobalIndex(int depth, uint matchorderindex, searchType flag) {
    // start=Get_Time();
    const std::vector<uint> &matchOrder = this->order_vs_[matchorderindex];
    uint queryVertex = matchOrder[depth];
    uint queryVertexLabel = this->query_.GetVertexLabel(queryVertex);
    // const auto & fneighbors=this->query_.forwardNeighbors[matchorderindex][depth];
    std::vector<SingleCandidate> &singleVertexCandidate = this->matchCandidate[depth];
    ...
    if (depth == query_.NumVertices() - 1) {
        ...
        for (int i = 0; i < singleVertexCandidate.size(); i++) {
            ...
            //copy SingleCandidate
            //updateweight;
            searchMatchesWithGLobalIndex(depth + 1, matchorderindex, flag);
            this->visited_[dataV] = false;
        }
        this->matchCandidate[depth].resize(0);
        this->match[depth].clearSingleCandidate();
...
void Graphflow::densityFilter(uint matchorder_index, uint depth, std::vector<SingleCandidate> &singleVertexCandidate) {
    ...
    float backWeight;
#ifdef LOCAL
    backWeight=GetBackWeight(matchorder_index, depth + 1);
//global or globalEdgeIndex
#else
    backWeight=suffixMax[depth+1];
#endif
...
void Graphflow::combination_helper(std::vector<std::vector<int>> &result, std::vector<int> &current,
    ...
    for (int i = 0; i < nums[k].size(); i++) { // ??? k+1 ????????????????
        current.push_back(nums[k][i]);
        combination_helper(result, current, nums, k + 1); // ??¯Ñ???????????
        current.pop_back();
    }
}
...
void Graphflow::setSingleVertexByIntersectionResult(std::vector<tuple<int, int, float>> &singleVertex,
    ...
    //intersectresult?????r??????LD?????????singleVertex?????????candidate?????
    //????intersectresult???????singleVertex
   ...
   std::cout<<endl;
    ...
    /* auto wt=findWeightAndTminBeforeIsolated();
     ...
     for(int j=0;j<combinezIsolateVertexs.size();j++){
         matchresult.push_back(make_pair(std::get<0>(combinezIsolateVertexs[j][0]),0));//id????????????ß÷?????¶À??
     }
     //?????????
      ...
      que.push(matchresult);
         while(!que.empty()) {
             ...
             if (ms.size() == isrepeat.size()) {
                 ...
                 recoverIsolateVertexMatchResult(isolateVertexs);
                 //?????????ß÷?????ß≥???ùI??
                 std::map<float, vector<std::pair<int, int>>> minWeight;//?ß‹??????¶À??
                 ...
                 //????????????????????????key??????diff?????ß≥???ùI?????????????????????????
                 std::map<float, vector<std::pair<int, int>>> minWeight;//?ß‹??????¶À??
                 std::unordered_map<int,vector<pair<int,int>>>sumId;//id??????ß‹?????????
                 std::unordered_map<int,float>idMaxDiff;//id?????????
                 for(int i=0;i<ms.size();i++){
                     int id=ms[i].first;
                     int index=ms[i].second;
...
bool Graphflow::updaterightNeighborCandidate(int matchorderindex, uint uk, uint uk_neigh, bool isFirstEdge, uint vk,
    ...
    for (int i = 0; i < n; i++) {
        ...
        if(i>0){
            ...
            lower = std::lower_bound(vN.begin(), vN.end(), neighbor, CompareNeighbors2);
        }
...
void Graphflow::PrintAverageTime(int len) {
    ...
    std::cout<<"test1 "<<test1.GetTimer()*1.0/len<<endl;
...
void Graphflow::deleteUpdateglobalVertexStarIndexWithGlobal(uint u1, uint v1, uint n) {
    uint u1label = query_.GetVertexLabel(u1);
    for (int j = 0; j < n; j++) {
        int vertex_index = order_vertex_index[j][u1];
        if (vertex_index == 0)
...
void Graphflow::deleteUpdateStarIndexWithGlobal(uint v1, uint v2, std::vector<int> &match) {
    uint n = query_.NumEdges();
    ...
    for (auto it = match.begin(); it != match.end(); it++) {
        ...
        uint elabel = std::get<2>(query_.GetEdgeLabel(u1, u2));
        if (v1label != v2label) {
            deleteUpdateglobalVertexStarIndexWithGlobal(u1, v1, n);
            deleteUpdateglobalVertexStarIndexWithGlobal(u2, v2, n);
        } else {
            for (int i = 0; i < 2; i++) {
                deleteUpdateglobalVertexStarIndexWithGlobal(u1, v1, n);
                deleteUpdateglobalVertexStarIndexWithGlobal(u2, v2, n);
...
void Graphflow::deleteUpdateStarIndexWithEdgeIndex(uint v1, uint v2, std::vector<int> &match) {
    deleteUpdateGLobalEdgeIndex(v1,v2,match);
}

void Graphflow::deleteUpdateGLobalEdgeIndex(uint v1, uint v2, std::vector<int> &match) {
    uint u1label=data_.GetVertexLabel(v1);
    uint u2label=data_.GetVertexLabel(v2);
    uint elabel=std::get<2>(data_.GetEdgeLabel(v1,v2));
    if (u1label > u2label)
        swap(u1label, u2label);
    if(v1>v2)
        swap(v1,v2);
    std::tuple < uint, uint, uint > key(u1label, u2label, elabel);
    std::pair<uint,uint>pair(v1,v2);
    auto it2 = data_.globalEdgeTodataVertex.find(key);
    if (it2 != data_.globalEdgeTodataVertex.end() && it2->second == pair) {
        data_.globalEdgeIndex[key] = 0;
        //update data with alldataedge matches <u1label,u2label,elabel>
        for (int i = 0; i < data_.NumVertices(); i++) {
            const std::vector<uint> &neighbors = data_.GetNeighbors(i);
            auto lower = std::lower_bound(neighbors.begin(), neighbors.end(), i);
            while (lower != neighbors.end()) {
                uint vv1 = i;
                uint vv2 = (*lower);
                uint v1label = data_.GetVertexLabel(vv1);
                uint v2label = data_.GetVertexLabel(vv2);
                uint label = std::get<2>(data_.GetEdgeLabel(vv1, vv2));
                float weight = data_.GetEdgeWeight(vv1, vv2);
                if (v1label > v2label)
                    std::swap(vv1, vv2);
                auto tuple_v = std::make_tuple(v1label, v2label, label);
                if (tuple_v == key) {
                    if(data_.globalEdgeIndex[key]<weight){
                        data_.globalEdgeIndex[key]=weight;
                        std::pair<uint,int>pair_v(vv1,vv2);
                        data_.globalEdgeTodataVertex[key]=pair_v;
                    }
                }
            }
        ...
        //update in matches
        uint candidate_u_pos;
        uint candidate_u_neighbor;
        uint candidate_u;
        for(int i=0;i<match.size();i++){
            uint u1 = order_vs_[match[i]][0];
            uint u2 = order_vs_[match[i]][1];
            ...
            uint u2label = query_.GetVertexLabel(u2);
            if (u1label > u2label) {
                std::swap(u1label, u2label);
            }
            uint elabel = std::get<2>(query_.GetEdgeLabel(u1, u2));
            for (int j = 0; j < query_.NumEdges(); j++) {
                uint u1pos = order_vertex_index[j][u1];
                uint u2pos = order_vertex_index[j][u2];
                if (u1pos > u2pos) {
                    candidate_u_pos = u1pos;
                    candidate_u = u1;
                    candidate_u_neighbor = u2;
                } else {
                    candidate_u_pos = u2pos;
                    candidate_u = u2;
                    candidate_u_neighbor = u1;
                }
                uint v1label=data_.GetVertexLabel(candidate_u);
                StarGraph*s=globalStarIndex[j][candidate_u_pos];
                std::vector<ForwardNeighbor*>&queryVertex=s->GetqueryVertex();
                float sumweight=0;
                for(auto fn:queryVertex){
                    uint v2label=fn->GetVertexLabel();
                    uint label=fn->GetElabel();
                    if(v1label>v2label)
                        swap(v1label,v2label);
                    float maxweight=data_.globalEdgeIndex[std::make_tuple(v1label,v2label,elabel)];
                    sumweight+=maxweight;
                }
                s->setStarMaxWeight(sumweight);
...
void Graphflow::SearchMatchesWithEdge(uint m,uint v1,uint v2,uint weight,uint u1,uint u2,searchType type){
    if(isInsert)
        IsearchSpace+=2;
    else
        DsearchSpace+=2;
...
void Graphflow::createGlobalEdgeIndex() {
    uint m=query_.NumEdges();
    uint n=query_.NumVertices();
    for(int i=0;i<m;i++){
        for(int j=1;j<n;j++){
            uint candidate_v=order_vs_[i][j];
            uint v1label=query_.GetVertexLabel(candidate_v);
            StarGraph *s=globalStarIndex[i][j];
            std::vector<ForwardNeighbor *> &queryVetex = s->GetqueryVertex();
            float sumweight=0;
            for(auto fneighbor:queryVetex){
                uint v2label=fneighbor->GetVertexLabel();
                uint label=fneighbor->GetElabel();
                if(v1label>v2label)
                    std::swap(v1label,v2label);
                float maxweight= data_.globalEdgeIndex[std::make_tuple(v1label,v2label,label)];
                //fneighbor->setMaxWeight(maxweight);
                sumweight+=maxweight;
            }
            if (s->getStarMaxWeight() == queryVetex.size() * mw || s->getStarMaxWeight() < sumweight) {
                s->setStarMaxWeight(sumweight);
...
void Graphflow::updateGlobalEdgeIndexInMatches(std::vector<int>&match,uint v1,uint v2,float weight) {
    uint candidate_u_pos;
    uint candidate_u_neighbor;
    uint candidate_u;
    for (int i = 0; i < match.size(); i++) {
        ...
        for (int j = 0; j < query_.NumEdges(); j++) {
            ...
            updateGlobalEdgeIndex(match[i], candidate_u, candidate_u_pos, v1, v2, candidate_u_neighbor, weight);
        }
    }
}