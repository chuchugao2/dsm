...
Graphflow::Graphflow(Graph& query_graph, Graph& data_graph,
        ...
        ,topKSet(0)
        ,allMatchRecords(0)
        ,sameLabelVertex(query_graph.NumVertices())
{
...
void Graphflow::searchMatches(uint matchorderindex, searchType flag)  {
    ...
    //2.1若是其前向邻居中有LD节点，展开，利用LD节点的候选解递归 不用
 /*   if(!LDVertexs.empty()){
...
//    std::sort(candidate.begin(), candidate.end(), greater());
   ...
   /* float LDWeight=0;
    ...
    while(iter!=singleVertexCandidate.end()){
        float tmpweight=std::get<2>(this->match[preIndex])+std::get<2>(*iter)+ backWeight+isolateWeight;
...
/*    for(int i=0;i<len;i++)
    ...
    while(!isover){
        ...
        /*if(hash[next_vertex_group]>combinezIsolateVertexs[next_vertex_group].size())
        {
            for(int i=0;i<len;i++)
                std::cout<<noscan[i]<<" ";
            std::cout<<endl;
            for(int i=0;i<len;i++)
                std::cout<<hash[i]<<" ";
            std::cout<<endl;
            std::cout<<endl<<"hash out of range"<<endl;
            std::cout<<"hash["<<next_vertex_group<<"] "<<hash[next_vertex_group]<<" combine size"<<combinezIsolateVertexs[next_vertex_group].size()<<endl;
        }*/
        ...
        //std::cout<<"next vertex group: "<<next_vertex_group<<" hash[next]: "<<hash[next_vertex_group]<<"T["<<next_vertex_group<<"]:"<<TboundNext<<endl;
        if(TboundNext>Tmax)
...
void Graphflow::createSameLabelVertex() {
    for(int i=0;i<query_.NumVertices();i++)
    {
        for(int j=0;j<query_.NumVertices();j++){
            if(query_.GetVertexLabel(j)==query_.GetVertexLabel(i)){
                sameLabelVertex[i].emplace_back(j);
            }
        }
    }
}
