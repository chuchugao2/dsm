...
Instopk::Instopk(Graph &query_graph, Graph &data_graph,Subgraph &subgraph,  uint max_num_results, bool print_prep, bool print_enum,
                 bool homo, uint d) : matching(query_graph, data_graph, subgraph,max_num_results,
                                      ...
                                      dToOrderType(d + 1), TopologyIndex(d + 1), queryTopologyIndex(d + 1), MNW(d + 1) {
    for (int i = 0; i < d + 1; i++) {
...
void Instopk::AddEdge(uint v1, uint v2, uint label, float weight, uint timestamp) {
    ...
    updateMNWIndexAndDataTopologyIndex();
    //3.searchTopkResult
    //3.1 updateQueryCandidate
    updateQueryCandidate();
    ...
    */
                        ...
                        if (edgeIDs2.size() != 0) {
                            for (int k: edgeIDs2) {
                                ...
                                if (sortEdgeList[nextEdgeType][k].GetV2() != node2) {
                                    ee = std::to_string(sortEdgeList[nextEdgeType][k].GetV2()) + "#" +
                                         std::to_string(sortEdgeList[nextEdgeType][k].GetV1());
                                    ree = std::to_string(sortEdgeList[nextEdgeType][k].GetV1()) + "#" +
                                          std::to_string(sortEdgeList[nextEdgeType][k].GetV2());
                                } else {
                                    ee = std::to_string(sortEdgeList[nextEdgeType][k].GetV1()) + "#" +
                                         std::to_string(sortEdgeList[nextEdgeType][k].GetV2());
                                    ree = std::to_string(sortEdgeList[nextEdgeType][k].GetV2()) + "#" +
                                          std::to_string(sortEdgeList[nextEdgeType][k].GetV1());
                                }
        ...
        for (int c = pointers[max] + 1; c < list.size(); c++) {
            ...
            for (Edge ee: arr1) {
                int v1 = ee.GetV1();
                int v2 = ee.GetV2();
                bool v1flag1 = std::find(matchVertexCandidate[v1].begin(), matchVertexCandidate[v1].end(), n1) !=
                               matchVertexCandidate[v1].end() ? 1 : 0;
                bool v1flag2 = std::find(matchVertexCandidate[v2].begin(), matchVertexCandidate[v2].end(), n2) !=
                               matchVertexCandidate[v2].end() ? 1 : 0;
                bool v2flag1 = std::find(matchVertexCandidate[v1].begin(), matchVertexCandidate[v1].end(), n2) !=
                               matchVertexCandidate[v1].end() ? 1 : 0;
                bool v2flag2 = std::find(matchVertexCandidate[v2].begin(), matchVertexCandidate[v2].end(), n1) !=
                               matchVertexCandidate[v2].end() ? 1 : 0;
        ...
        if (topKSet.size() == k) {
            ...
            for (int i = 0; i < query_.vEdge.size(); i++) {
                std::string edge =
                        std::to_string(query_.vEdge[i].GetV1()) + "#" + std::to_string(query_.vEdge[i].GetV2());
...
void Instopk::updateSortEdgelist(uint v1, uint v2) {
...
void Instopk::updateMNWIndexAndDataTopologyIndex() {
    //clear MNWindex and TopologyIndex
    for (auto &vec: TopologyIndex) {
        for (auto &map: vec) {
            for (auto &pair: map) {
                pair.second = 0;
            }
...
void Instopk::updateQueryCandidate() {
    int n = matchVertexCandidate.size();
    for (int i = 0; i < n; i++) {
        matchVertexCandidate[i].resize(0);
    }
    InitialqueryCandidate();
}
