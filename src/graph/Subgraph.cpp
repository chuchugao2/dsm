...
//找到大于等于v2的迭代器的位置
    SubNeighbor neighbor(v2,v2label,label,weight,u2,u1);
    if(setVNeighbors[v1].count(neighbor)){
        return false;
    }
    else{
        auto lower=std::lower_bound(vNeighbors[v1].begin(),vNeighbors[v1].end(),neighbor,std::greater<SubNeighbor>());
        vNeighbors[v1].insert(lower,neighbor);
        //vNeighbors[v1].emplace_back(neighbor);
        setVNeighbors[v1].insert(neighbor);
    }
    SubNeighbor neighbor2(v1, v1label,label,weight,u1,u2);
    if(setVNeighbors[v2].count(neighbor2)){
        return false;
    }
    else{
        auto lower=std::lower_bound(vNeighbors[v2].begin(),vNeighbors[v2].end(),neighbor2,std::greater<SubNeighbor>());
...
{
    SubNeighbor neighbor(v2,v2label,elabel,weight,u2,u1);
    if(!setVNeighbors[v1].count(neighbor)){
        return false;
    }
    auto lower=std::lower_bound(vNeighbors[v1].begin(),vNeighbors[v1].end(),neighbor,std::greater<SubNeighbor>());
    ...
    SubNeighbor neighbor2(v1,v1label,elabel,weight,u1,u2);
    if(!setVNeighbors[v2].count(neighbor2)){
        return false;
    }
    auto lower2=std::lower_bound(vNeighbors[v2].begin(),vNeighbors[v2].end(),neighbor2,std::greater<SubNeighbor>());
...
const std::vector<SubNeighbor>& Subgraph::GetVNeighbors(uint v) const {
    return vNeighbors[v];
}
